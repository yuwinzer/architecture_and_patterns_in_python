...
class Database:
    ...
    def __init__(self):
        ...
        self.courses_users = db_mapper.Table('courses_users')
        self.lines = db_mapper.Table('lines')
        self.types = db_mapper.Table('line_types')

        self.lines_list = self.lines.get_all()
        # print(f'{self.lines_list=}')
        self.courses_list = self.courses.get_all()
        # print(f'{self.courses_list=}')
        self.db_precount_courses_for_lines()
    ...
    def get_lines_by_parent(self, _id: int):
    ...
    def db_get_course_amt_by_line(self, _line) -> int:
        # line = _to_int(line)
        _count = 0
        for _elem in self.courses_list:
            if _elem.line == _line:
                _count = _count + 1
        # print(f'{count=}')
        return _count
